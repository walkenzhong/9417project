...
def each_student_data(filename):
    ...
    student_uid = re.findall('dark_([^"]*).csv', filename)[0]
...
def main(file_dir):
    ...
    for i in all_filename:
        ...
        #print(paths)
        if 'dark' not in file_in_paths :
            os.mkdir('data20191117\\dark\\')
        student.to_csv('data20191117\\dark\\dark_'+ student_uid + '.csv', encoding='gbk')
        print(student_uid)
    print('done')
...
def show_plt(uid, student):
    ...
    plt.savefig('data20191117\\dark\\dark_' + uid + ".png")
    # plt.show()


if __name__ == '__main__':
    main('..\\input\\sensor\\dark\\')